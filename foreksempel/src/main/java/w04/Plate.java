...
public class Plate {
	...
	/**
	 * Den bruker regex, som er på utsiden av pensum, jeg har lagt ved en SVÆRT
	 * enkel metode som gjør det annerledes like under.
	 * 
	...
	private static boolean checkSignNonRegex(String plate) {
		if (Character.isLetter(plate.charAt(0)) && Character.isLetter(plate.charAt(1))
				&& Character.isDigit(plate.charAt(2)) && Character.isDigit(plate.charAt(3))
				&& Character.isDigit(plate.charAt(4)) && Character.isDigit(plate.charAt(5))
				&& Character.isDigit(plate.charAt(6)) && plate.length() == 7) {
			return true;
		}
		return false;
	}
