...
public class CarOrig {
	...
	public boolean addHiker(PersonOrig person) {
		...
		if (hikers.size() == seats - 1) {
			System.out.println("Denna bilen er full, " + person.getName() + " fikk ikke plass!");
			return false;
		} else {
			hikers.add(person);
			System.out.println(person.getName() + " fikk plass.");
			return true;
		}
	}
	...
	public CarOrig(String plate, int seats) {
		...
		this.seats = seats;
		System.out.println("Bil med regnr " + this.plate.getSign() + " og " + this.seats + " seter.");
	}
	...
	public static void main(String[] args) {
		...
		car1.hikers.stream().filter(p -> p.getAge() >= 18).sorted((p1, p2) -> Integer.compare(p1.getAge(), p2.getAge()))
