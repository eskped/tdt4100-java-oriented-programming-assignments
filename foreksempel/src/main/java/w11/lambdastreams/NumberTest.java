...
public class NumberTest {
	...
	/*
	 * Poenget her er at en ved å splitte opp if-sjekken ovenfor ved hjelp av
	 * streams, og egne metoder, så vil en gjøre koden mye mer lesbar.
	 */
	public static int findSquareOfMaxOdd2(List<Integer> numbers) {
		return numbers.stream().filter(NumberTest::isOdd) // Predicate is functional interface and
				...
				.max(Comparator.naturalOrder()).map(i -> i * i).get(); // Returner den.
	}
