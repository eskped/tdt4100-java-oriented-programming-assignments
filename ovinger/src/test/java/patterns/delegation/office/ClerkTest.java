...
public class ClerkTest {
	...
	public void testPrintDocuments() {
		...
		Assertions.assertTrue(printer.getPrintHistory(clerk).contains("dokument1"), "Teste om dokumentet som ble printet ble lagt til i printerhistorikken");
		Assertions.assertEquals(1, clerk.getTaskCount(), "Teste antall oppgaver etter 1 utskrift");
		Assertions.assertEquals(1, printer.getPrintHistory(clerk).size(), "Teste antall utskrifter i historikken etter 1 utskrift");
		
		
		// Printer enda et dokument
		clerk.printDocument("dokument2");
		Assertions.assertTrue(printer.getPrintHistory(clerk).contains("dokument2"), "Teste om dokument 2 som ble printet ble lagt til i printerhistorikken");
		Assertions.assertEquals(2, clerk.getTaskCount(), "Teste antall oppgaver etter 2 utskrifter");
		Assertions.assertEquals(2, printer.getPrintHistory(clerk).size(), "Teste antall utskrifter i historikken etter 2 utskrifter");
	}
